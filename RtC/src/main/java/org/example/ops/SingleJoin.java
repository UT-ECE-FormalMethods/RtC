...
public class SingleJoin {
    ...
    public ConstraintAutomaton joinAutomataByStateChecking(ConstraintAutomaton automaton_1, ConstraintAutomaton automaton_2) throws JoinOperationFailedException {
                    ...
                    if (!Objects.equals(state1.getId(), state2.getId())) {
                        ...
                        if ((Objects.equals(state1.getComposition().get(0), state2.getComposition().get(0)))) {
                            ...
                            if (transition != null) {
                                ...
                                boolean intersectionExists = automatonUtils.transitionsIntersectionExists(transition.getLabels(), automaton_1.getAlphabet());
                                if (!intersectionExists) {
                                    Transition newTransition = new Transition();
                                    newTransition.setSource(state1.getId());
                                    newTransition.setTarget(state2.getId());
                                    newTransition.setLabels(transition.getLabels());
                        ...
                        //e.g. this if is for the case of transition from q1_q2 -> p1_q2 (second q2 is same)
                            ...
                            if (transition != null) {
                                boolean intersectionExists = automatonUtils.transitionsIntersectionExists(transition.getLabels(), automaton_2.getAlphabet());
                                if (!intersectionExists) {
                                    Transition newTransition = new Transition();
                                    newTransition.setSource(state1.getId());
                                    newTransition.setTarget(state2.getId());
                        ...
                        // this case is for when neither the first nor the second are the same (e.g. q1_p2 -> p1_q2)
                            ...
                            if (transition1 != null && transition2 != null) { // check this !!!
                                ArrayList<String> intersection1 = automatonUtils.getTransitionLabelsIntersection(transition1.getLabels(), automaton_2.getAlphabet());
                                ArrayList<String> intersection2 = automatonUtils.getTransitionLabelsIntersection(transition2.getLabels(), automaton_1.getAlphabet());
                                if (intersection1.equals(intersection2)) {
                                    Transition newTransition = new Transition();
                                    newTransition.setSource(state1.getId());
                                    newTransition.setTarget(state2.getId());
                                    newTransition.setLabels(automatonUtils.getTransitionLabelsUnion(transition1.getLabels(), transition2.getLabels()));
                                    newTransition.setConstraints(new ArrayList<>());
                                    joinedTransitions.add(newTransition);
                                }
                            }
    ...
    public ConstraintAutomaton joinAutomataByTransitionChecking(ConstraintAutomaton automaton1, ConstraintAutomaton automaton2) throws JoinOperationFailedException {
        ConstraintAutomaton joinedAutomaton = new ConstraintAutomaton();
        joinedAutomaton.setId(automatonUtils.getJoinedAutomatonId(automaton1.getId(), automaton2.getId()));
        List<State> states1 = automaton1.getStates();
        List<State> states2 = automaton2.getStates();
        HashMap<String, List<Transition>> automaton1StateTransitionsMap = automatonUtils.createStateTransitionMap(automaton1);
        HashMap<String, List<Transition>> automaton2StateTransitionsMap = automatonUtils.createStateTransitionMap(automaton2);
        ...
        try {
            for(State state1: states1) {
                List<Transition> state1TransitionsList = automaton1StateTransitionsMap.get(state1.getId());
                for(State state2: states2) {
                    List<Transition> state2TransitionsList = automaton2StateTransitionsMap.get(state2.getId());

                    for(Transition transition1: state1TransitionsList) {
                        for(Transition transition2: state2TransitionsList) {
                            ArrayList<String> automaton1Intersection = automatonUtils.getTransitionLabelsIntersection(transition1.getLabels(), automaton2.getAlphabet());
                            ArrayList<String> automaton2Intersection = automatonUtils.getTransitionLabelsIntersection(transition2.getLabels(), automaton1.getAlphabet());

                            if(automaton1Intersection.equals(automaton2Intersection)) {
                                Transition transition = new Transition();
                                transition.setSource(state1.getId() + "_" + state2.getId());
                                transition.setTarget(transition1.getTarget() + "_" + transition2.getTarget());
                                transition.setLabels(automatonUtils.getTransitionLabelsUnion(transition1.getLabels(), transition2.getLabels()));
                                String transitionId = automatonUtils.getJoinedTransitionId(transition);
                                if(joinedTransitionsIds.contains(transitionId)) {
                                    joinedTransitionsIds.add(transitionId);
                                    joinedTransitions.add(transition);
                                }

                                State stateSource = new State();
                                stateSource.setId(state1.getId() + "_" + state2.getId());
                                stateSource.setComposition(new ArrayList<String>(Arrays.asList(state1.getId() + "_" + state2.getId())));
                                stateSource.setInitial(state1.isInitial() && state2.isInitial());
                                if(joinedStatesIds.contains(stateSource.getId())) {
                                    joinedStatesIds.add(stateSource.getId());
                                    joinedStates.add(stateSource);
                                }

                                State stateTarget = new State();
                                stateTarget.setId(transition1.getTarget() + "_" + transition2.getTarget());
                                stateTarget.setComposition(new ArrayList<String>(Arrays.asList(transition1.getTarget() + "_" + transition2.getTarget())));
                                stateTarget.setInitial(state1.isInitial() && state2.isInitial());
                                if(joinedStatesIds.contains(stateTarget.getId())) {
                                    joinedStatesIds.add(stateTarget.getId());
                                    joinedStates.add(stateTarget);
                                }
                            }
                        }
                    }

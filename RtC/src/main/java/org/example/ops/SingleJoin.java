...
public class SingleJoin {
    ...
    public ConstraintAutomaton joinAutomataByStateChecking2(ConstraintAutomaton automaton_1, ConstraintAutomaton automaton_2) throws JoinOperationFailedException {
        ConstraintAutomaton joinedAutomaton = new ConstraintAutomaton();
        joinedAutomaton.setId(automatonUtils.getJoinedAutomatonId(automaton_1.getId(), automaton_2.getId()));
        List<State> states1 = automaton_1.getStates();
        List<State> states2 = automaton_2.getStates();
...
//            }
            ...
            return AutomatonUtils.removeUnreachableStates(joinedAutomaton);
        }
        catch (Exception e) {
            throw new JoinOperationFailedException(e.getMessage());
        }
                ...
                for(State state2: states2) {
                    ...
                    for(Transition transition1: state1TransitionsList) {
                        for(Transition transition2: state2TransitionsList) {
                            ArrayList<String> automaton1Intersection = automatonUtils.getTransitionLabelsIntersection(transition1.getLabel(), automaton2.getAlphabet());
                            ArrayList<String> automaton2Intersection = automatonUtils.getTransitionLabelsIntersection(transition2.getLabel(), automaton1.getAlphabet());

                            if(automatonUtils.intersectionsAreEqual(automaton1Intersection, automaton2Intersection)) {
                                ...
                                transition.setLabel(automatonUtils.getTransitionLabelsUnion(transition1.getLabel(), transition2.getLabel()));
